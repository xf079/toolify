...
export class MainBrowser {
  ...
  private createBuiltPluginView(item: IPlugin) {
    return new Promise((resolve) => {
      ...
      loadSystemContentsUrl(pluginView.webContents, item.main);

      pluginView.webContents.on('did-finish-load', () => {
        // 插件加载成功
        resolve(true);
        this.setWindowToPluginRect();
      });
  ...
  async openPlugin(plugin: IPlugin) {
